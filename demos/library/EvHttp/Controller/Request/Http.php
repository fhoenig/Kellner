...
/** EvHttp_Controller_Request_Abstract */
require_once 'EvHttp/Controller/Request/Abstract.php';

/** Zend_Uri */
require_once 'Zend/Uri.php';

/**
 * EvHttp_Controller_Request_Http
 *
 * HTTP request object for use with EvHttp_Controller family.
 *
 * @uses EvHttp_Controller_Request_Abstract
 * @package EvHttp_Controller
 * @subpackage Request
 */
class EvHttp_Controller_Request_Http extends EvHttp_Controller_Request_Abstract
{
    /**
     * Scheme for http
     *
     ...
     * @return void
     * @throws EvHttp_Controller_Request_Exception when invalid URI passed
     */
	...
	public function init($uri = null)
	{
		...
		if (null !== $uri) {
            ...
            if ($uri->valid()) {
                ...
                require_once 'EvHttp/Controller/Request/Exception.php';
                throw new EvHttp_Controller_Request_Exception('Invalid URI provided to constructor');
            }
        } else {
            $this->setRequestUri();
        }
     ...
     * @param mixed $value
     * @return void
     * @throws EvHttp_Controller_Request_Exception
     */
    public function __set($key, $value)
    {
        require_once 'EvHttp/Controller/Request/Exception.php';
        throw new EvHttp_Controller_Request_Exception('Setting values in superglobals not allowed; please use setParam()');
    }
    ...
    public function setQuery($spec, $value = null)
    {
        if ((null === $value) && !is_array($spec)) {
            ...
            throw new EvHttp_Controller_Exception('Invalid value passed to setQuery(); must be either array of values or key/value pair');
        }
        if ((null === $value) && is_array($spec)) {
    ...
    public function setPost($spec, $value = null)
    {
        if ((null === $value) && !is_array($spec)) {
            require_once 'EvHttp/Controller/Exception.php';
            throw new EvHttp_Controller_Exception('Invalid value passed to setPost(); must be either array of values or key/value pair');
        }
        if ((null === $value) && is_array($spec)) {
            foreach ($spec as $key => $value) {
    ...
    public function getHeader($header)
    {
        if (empty($header)) {
            require_once 'EvHttp/Controller/Request/Exception.php';
            throw new EvHttp_Controller_Request_Exception('An HTTP header name is required');
        }
