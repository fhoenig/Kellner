 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class EvHttp_Controller_Plugin_Broker extends EvHttp_Controller_Plugin_Abstract
{

    /**
     * Array of instance of objects extending EvHttp_Controller_Plugin_Abstract
     *
     * @var array
     */
     ...
     * @return EvHttp_Controller_Plugin_Broker
     */
    public function registerPlugin(EvHttp_Controller_Plugin_Abstract $plugin, $stackIndex = null)
    {
        if (false !== array_search($plugin, $this->_plugins, true)) {
            ...
            throw new EvHttp_Controller_Exception('Plugin already registered');
        }
        ...
        if ($stackIndex) {
            if (isset($this->_plugins[$stackIndex])) {
                require_once 'EvHttp/Controller/Exception.php';
                throw new EvHttp_Controller_Exception('Plugin with stackIndex "' . $stackIndex . '" already registered');
            }
     ...
     * Unregister a plugin.
     *
     * @param string|EvHttp_Controller_Plugin_Abstract $plugin Plugin object or class name
     * @return EvHttp_Controller_Plugin_Broker
     */
    public function unregisterPlugin($plugin)
    {
        if ($plugin instanceof EvHttp_Controller_Plugin_Abstract) {
            // Given a plugin object, find it in the array
            $key = array_search($plugin, $this->_plugins, true);
            if (false === $key) {
                require_once 'EvHttp/Controller/Exception.php';
                throw new EvHttp_Controller_Exception('Plugin never registered.');
            }
     ...
     * @return false|EvHttp_Controller_Plugin_Abstract|array Returns false if none found, plugin if only one found, and array of plugins if multiple plugins of same class found
     */
    ...
    /**
     * Called before EvHttp_Controller_Front exits its iterations over
     * the route set.
     *
     * @param  EvHttp_Controller_Request_Abstract $request
     * @return void
     ...
     *
     * During the dispatch loop, EvHttp_Controller_Front keeps a
     * EvHttp_Controller_Request_Abstract object, and uses
     * EvHttp_Controller_Dispatcher to dispatch the
     * EvHttp_Controller_Request_Abstract object to controllers/actions.
     *
     * @param  EvHttp_Controller_Request_Abstract $request
     * @return void
     */
