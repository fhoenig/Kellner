...
/**
 * @see EvHttp_Controller_Action_Interface
 */
require_once 'EvHttp/Controller/Action/Interface.php';

/**
 * @see EvHttp_Controller_Front
 */
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class EvHttp_Controller_Action implements EvHttp_Controller_Action_Interface
{
    /**
    ...
    /**
     * EvHttp_Controller_Request_Abstract object wrapping the request environment
     * @var EvHttp_Controller_Request_Abstract
     */
    protected $_request = null;

    /**
     * EvHttp_Controller_Response_Abstract object wrapping the response
     ...
     *
     * @var EvHttp_Controller_Action_HelperBroker
     */
     ...
     * @return void
     */
    public function __construct(EvHttp_Controller_Request_Abstract $request, EvHttp_Controller_Response_Abstract $response, array $invokeArgs = array())
    {
        $this->setRequest($request)
             ->setResponse($response)
             ->_setInvokeArgs($invokeArgs);
        $this->_helper = new EvHttp_Controller_Action_HelperBroker($this);
        $this->init();
    }
     ...
     * @throws EvHttp_Controller_Exception if base view directory does not exist
     */
    public function initView()
    {
        ...
        if (!file_exists($baseDir) || !is_dir($baseDir)) {
            ...
            throw new EvHttp_Controller_Exception('Missing base view directory ("' . $baseDir . '")');
        }
     ...
     *
     * @see EvHttp_Controller_Response_Abstract::appendBody()
     ...
     * @param  bool $noController  Defaults to false; i.e. use controller name as subdir in which to search for view script
     * @return string
     * @throws EvHttp_Controller_Exception with bad $action
     */
    public function getViewScript($action = null, $noController = null)
    {
        ...
        if (null === $action) {
            ...
            require_once 'EvHttp/Controller/Exception.php';
            throw new EvHttp_Controller_Exception('Invalid action specifier for view render');
        }

        if (null === $this->_delimiters) {
            $dispatcher = EvHttp_Controller_Front::getInstance()->getDispatcher();
     ...
     *
     * @return EvHttp_Controller_Request_Abstract
     */
     ...
     *
     * @param EvHttp_Controller_Request_Abstract $request
     * @return EvHttp_Controller_Action
     */
     ...
     *
     * @return EvHttp_Controller_Response_Abstract
     */
     ...
     * @return EvHttp_Controller_Action_Helper_Abstract
     */
     ...
     *
     * @param EvHttp_Controller_Front $front
     * @return EvHttp_Controller_Action
     */
    public function setFrontController(EvHttp_Controller_Front $front)
    {
     ...
     * {@link EvHttp_Controller_Front}, it may modify the
    ...
    public function __call($methodName, $args)
    {
        ...
        if ('Action' == substr($methodName, -6)) {
            ...
            throw new EvHttp_Controller_Action_Exception(sprintf('Action "%s" does not exist and was not trapped in __call()', $action), 404);
        }

        throw new EvHttp_Controller_Action_Exception(sprintf('Method "%s" does not exist and was not trapped in __call()', $methodName), 500);
    }
     ...
     * request.
     *
     * Returns a EvHttp_Controller_Response_Abstract object, instantiating one
     ...
     *
     * @param null|EvHttp_Controller_Request_Abstract $request Optional request
     * object to use
     * @param null|EvHttp_Controller_Response_Abstract $response Optional response
     * object to use
     * @return EvHttp_Controller_Response_Abstract
     */
    public function run(EvHttp_Controller_Request_Abstract $request = null, EvHttp_Controller_Response_Abstract $response = null)
    {
        if (null !== $request) {
            $this->setRequest($request);
        } else {
     ...
     *
     * Proxies to {@link EvHttp_Controller_Action_Helper_Redirector::gotoUrl()}.
     *
     * @param string $url
