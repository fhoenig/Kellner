 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class EvHttp_Controller_Action_HelperBroker_PriorityStack implements IteratorAggregate, ArrayAccess, Countable
{
     ...
     */
    public function push(EvHttp_Controller_Action_Helper_Abstract $helper)
    {
    ...
    public function offsetGet($priorityOrHelperName)
    {
        if (!$this->offsetExists($priorityOrHelperName)) {
            ...
            throw new EvHttp_Controller_Action_Exception('A helper with priority ' . $priority . ' does not exist.');
        }
    ...
    public function offsetSet($priority, $helper)
    {
        ...
        if (!$helper instanceof EvHttp_Controller_Action_Helper_Abstract) {
            require_once 'EvHttp/Controller/Action/Exception.php';
            throw new EvHttp_Controller_Action_Exception('$helper must extend EvHttp_Controller_Action_Helper_Abstract.');
        }
    ...
    public function offsetUnset($priorityOrHelperName)
    {
        if (!$this->offsetExists($priorityOrHelperName)) {
            require_once 'EvHttp/Controller/Action/Exception.php';
            throw new EvHttp_Controller_Action_Exception('A helper with priority or name ' . $priorityOrHelperName . ' does not exist.');
        }
